...
export class Masterchef {
  ...
  async getStakingBalance(poolInfo: PoolInfo, address: string) {
    const user= await this.masterchef.methods.userInfo(poolInfo.poolId, address).call();
    const staking = {
  ...
  async getStakingReward(poolInfo: PoolInfo, address: string) {
    const pendingReward = await this.masterchef.methods.pendingCake(poolInfo.poolId, address).call();
    const reward = {
